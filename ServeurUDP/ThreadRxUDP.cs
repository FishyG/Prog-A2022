...
using System.Windows.Forms;  //pour utiliser le pointeur vers la form principale
...
namespace ServeurUDP
{
    internal class ThreadRxUDP
    {
        ...
        {
            if (ipClient != null)
                return "De IP = " + ipClient.ToString() + " (" + portClient.ToString() + "):   " + Encoding.Default.GetString(m_trameRx);// Encoding.ASCII.GetString(m_trameRx);
            else
                return "";
        }
